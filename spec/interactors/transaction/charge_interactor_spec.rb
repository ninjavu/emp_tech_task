...
RSpec.describe Transaction::ChargeInteractor, type: :interactor do
  ...
  describe 'ref transaction' do
    ...
    context 'when invalid params' do
      let(:params) { { transaction_reference: 'nil' } }
  ...
  describe 'other charge transactions existence' do
    context 'when exists' do
      ...
      it 'it returns error' do
        ctx = interactor.call
        expect(ctx.errors[:server]).to eq('charge transaction for this authorize transaction already exists')
      end
    end
  ...
  describe 'transaction creation' do
    context 'when valid params' do
      it 'it creates Charge transaction' do
        interactor.call
        expect(Transaction::Charge.count).to be(1)
      end
